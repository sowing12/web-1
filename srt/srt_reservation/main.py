...
class SRT:
    ...
    def go_search(self):
        # 기차 조회 페이지로 이동
        response = self.s.get('https://etk.srail.kr/hpg/hra/01/selectScheduleList.do')
        print(response)
    ...
    def book_ticket(self, standard_seat, i):
        ...
        if "예약하기" in standard_seat:
            ...
            if self.driver.find_elements(By.ID, 'isFalseGotoMain'):
                self.is_booked = True
                print("예약 성공")
                bot = telegram.Bot(token='5645473580:AAGmssXEnnovpI-boWTVUl_NCETf9uXwMK4')
                chat_id = 5660667329
    ...
    def reserve_ticket(self, reservation, i):
        if "신청하기" in reservation:
            ...
            bot = telegram.Bot(token='5645473580:AAGmssXEnnovpI-boWTVUl_NCETf9uXwMK4')
            chat_id = 5660667329
            bot.sendMessage(chat_id=chat_id, text="예약대기완료")
            self.is_booked = True
